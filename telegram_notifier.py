...
class TelegramNotifier:
    ...
    def _create_hourly_bars(self, hourly_data: Dict[int, int]) -> str:
        if not hourly_data:
    ...
    def _create_pnl_trend(self, pnl_trend: List[float]) -> str:
        if not pnl_trend or len(pnl_trend) < 2:
            return "Sin datos de PnL"
        
        step = max(1, len(pnl_trend) // 10)
        simplified = pnl_trend[::step][:10]
