...
# -*- coding: utf-8 -*-
"""
Zaffex bot — CoinEx spot (Railway-ready)
- RSI(14) con lógica de CRUCE + histéresis (evita re-entradas repetidas)
...
def lots_by_mode(mode: str) -> int:
    if mode == "agresivo":
        return CONFIG["LOT_SIZE_AGRESIVO"]
    if mode == "moderado":
        return CONFIG["LOT_SIZE_MODERADO"]
    if mode in ("conservador","conservativo"):
        return CONFIG["LOT_SIZE_CONSERVADOR"]
    return 0
...
# ---------------- Persistencia mínima ----------------
POSITIONS_FILE = "/tmp/zaffex_positions.json"
STATS_FILE = "/tmp/zaffex_stats.json"
RSI_STATE_FILE = "/tmp/zaffex_rsi_state.json"
...
# ---------------- Estado del bot ----------------
RUNNING = True
positions: Dict[Tuple[str, str], Dict] = {}  # (symbol, mode) -> pos dict
last_signal_time: Dict[str, float] = {}
rsi_state: Dict[str, str] = {}  # per symbol: 'above' | 'below'
...
def open_position(ex, markets, symbol: str, mode: str, context: str = ""):
    ...
    try:
        if notifier and notifier.enabled():
            try:
                notifier.send_trade_open_rich(
                    symbol=symbol, mode=mode, lots=lots_by_mode(mode), timeframe=CONFIG["TIMEFRAME"],
                    price=price, tp=tp, sl=sl, qty=qty, reason=context,
                    tp_pct=CONFIG["TP_PCT"], sl_pct=CONFIG["SL_PCT"],
                    account_balance=STATS.get("balance", 0.0) if STATS else None
                )
            except Exception:
...
def maybe_close_one(ex, key: Tuple[str, str]):
    ...
    try:
        if notifier and notifier.enabled():
            try:
                notifier.send_trade_close_rich(
                    symbol=symbol, mode=mode, reason=why,
                    gross=gross_pnl, fees=(entry_fee + exit_fee), pnl=pnl_net,
                    entry=pos['entry'], qty=pos['qty'], hold_sec=now_ts()-pos.get('opened_at', now_ts())
                )
            except Exception:
                notifier.send_trade_close(
                    symbol, mode, why, gross_pnl, (entry_fee + exit_fee), pnl_net,
                    entry=pos['entry'], qty=pos['qty'], hold_sec=now_ts()-pos.get('opened_at', now_ts())
                )
        else:
...
def main():
    ...
    try:
        ...
        try:
            ...
            positions.update({tuple(k.split("|", 1)): v for k, v in pos_loaded.items()})
            rsi_state.update(persisted.get("rsi_state", {}))
        except Exception:
            pass
        ...
        while RUNNING:
            ...
            for symbol in CONFIG["SYMBOLS"]:
                try:
                    ...
                    th = CONFIG["RSI_THRESHOLD"]
                    hyst = CONFIG.get("RSI_HYSTERESIS", 2.0)
                    prev = rsi_state.get(symbol, "above")
                    state = "below" if rsi_val < th else ("above" if rsi_val > th + hyst else prev)
                    rsi_state[symbol] = state

                    if now_ts() - last_sig >= CONFIG["SIGNAL_COOLDOWN"]:
                        if prev != "below" and state == "below":
