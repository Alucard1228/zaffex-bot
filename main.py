...
def debug_throttled(symbol: str, text: str, every_sec: float = 5.0):
    now = time.time(); last = _last_debug.get(symbol, 0.0)
    if now - last >= every_sec:
        print(f"[{ts()}] [DEBUG] {symbol} | {text}")
        _last_debug[symbol] = now

_lock_file = "/tmp/zaffex.lock"; _lock_fd = None
def acquire_lock() -> bool:
    global _lock_fd
    try:
        _lock_fd = os.open(_lock_file, os.O_CREAT | os.O_RDWR)
        import fcntl; fcntl.flock(_lock_fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
        try: os.ftruncate(_lock_fd, 0)
        except Exception: pass
        os.write(_lock_fd, str(os.getpid()).encode("utf-8"))
        return True
    except Exception as e:
        print(f"[{ts()}] [WARN] Another instance seems running (lock). {e}"); return False
...
def release_lock():
    ...
    try:
        if _lock_fd is not None:
            try:
                import fcntl; fcntl.flock(_lock_fd, fcntl.LOCK_UN)
            except Exception: pass
...
def _get(name: str, default: Optional[str] = None) -> Optional[str]:
    v = os.getenv(name, default); return v if (v is not None and v != "") else default
def _get_bool(name: str, default: str = "0") -> bool:
    return (_get(name, default) or "").strip().lower() in ("1","true","yes","y")
def _get_list(name: str, default: str = "") -> List[str]:
    raw = _get(name, default) or ""; return [s.strip() for s in raw.split(",") if s.strip()]
...
def _redact(s: str) -> str:
    if not s: return ""; return (s[:3]+"..."+s[-3:]) if len(s)>6 else "***"
...
def tg(msg: str):
    print(msg)
    try:
        if notifier and notifier.enabled(): notifier.send(msg)
    except Exception as _e:
...
def build_exchange():
    ...
    return ccxt.coinex({"apiKey": CONFIG["API_KEY"], "secret": CONFIG["API_SECRET"], "enableRateLimit": True, "options": {"defaultType": "spot"}})

def load_markets_retry(ex, retries: int = 5, delay: float = 2.0):
    for i in range(1, retries+1):
        try: return ex.load_markets()
        except Exception as e:
            print(f"[{ts()}] [WARN] load_markets {i}/{retries} -> {e}"); time.sleep(delay*i)
...
def size_from_notional(ex, markets, symbol: str, notional_cap: float) -> Tuple[float, float]:
    ...
    if min_qty and qty < min_qty:
        qty = float(min_qty); qty, price = clamp_qty_price(ex, symbol, qty, price)
    return qty, price

def notional_per_mode(mode: str) -> float:
    if mode == "agresivo": return CONFIG["CAPITAL_AGRESIVO"] / CONFIG["LOT_SIZE_AGRESIVO"]
    if mode == "moderado": return CONFIG["CAPITAL_MODERADO"] / CONFIG["LOT_SIZE_MODERADO"]
    if mode in ("conservador","conservativo"): return CONFIG["CAPITAL_CONSERVADOR"] / CONFIG["LOT_SIZE_CONSERVADOR"]
    return 0.0

STATS_FILE = "/tmp/zaffex_stats.json"; STATS = None
def _stats_default():
    return {"trades":0,"wins":0,"losses":0,"pnl":0.0,"fees":0.0,"volume":0.0,"balance":float(CONFIG.get("ACCOUNT_START",0.0)),
            "h_trades":0,"h_wins":0,"h_losses":0,"h_pnl":0.0,"h_fees":0.0,"h_volume":0.0,"h_started":0.0,"last_summary":0.0}
def load_stats():
    try:
        import json, os
    ...
    s = _stats_default(); s["h_started"]=time.time(); s["last_summary"]=time.time(); return s
def save_stats(s):
    try:
        import json
        with open(STATS_FILE,"w",encoding="utf-8") as f: json.dump(s,f)
    except Exception: pass
def stats_on_close(s, gross: float, fees: float, pnl: float, notional_exit: float):
    s["trades"]+=1; s["wins"]+=int(pnl>=0); s["losses"]+=int(pnl<0)
    s["pnl"]+=pnl; s["fees"]+=fees; s["volume"]+=abs(notional_exit)
    s["balance"]=float(CONFIG.get("ACCOUNT_START",0.0))+s["pnl"]
    s["h_trades"]+=1; s["h_wins"]+=int(pnl>=0); s["h_losses"]+=int(pnl<0)
    s["h_pnl"]+=pnl; s["h_fees"]+=fees; s["h_volume"]+=abs(notional_exit)
def maybe_send_summary(s):
    if not CONFIG.get("SUMMARY_ENABLED", True): return
    now=time.time(); every=int(CONFIG.get("SUMMARY_EVERY_MIN",60))*60
    if now - s.get("last_summary",0.0) >= every:
        window={"trades":s["h_trades"],"wins":s["h_wins"],"losses":s["h_losses"],"pnl":s["h_pnl"],"fees":s["h_fees"],"volume":s["h_volume"]}
        totals={"trades":s["trades"],"wins":s["wins"],"losses":s["losses"],"pnl":s["pnl"],"fees":s["fees"],"volume":s["volume"],"balance":s["balance"]}
        try:
            if notifier and notifier.enabled(): notifier.send_summary("last hour", window, totals)
        except Exception: pass
        s["h_trades"]=s["h_wins"]=s["h_losses"]=0; s["h_pnl"]=s["h_fees"]=s["h_volume"]=0.0
        s["h_started"]=now; s["last_summary"]=now; save_stats(s)

RUNNING=True; positions: Dict[str, Dict]={}; last_signal_time: Dict[str,float]={}; last_loss_time: Dict[Tuple[str,str],float]={}; equity=0.0
def signal_handler(sig, frame):
    global RUNNING; RUNNING=False; print(f"[{ts()}] [STOP] Shutdown signal received; stopping loop...")

def open_position(ex, markets, symbol: str, mode: str, context: str = ""):
    if not RUNNING: return
    key_cool=(symbol,mode)
    if now_ts()-last_loss_time.get(key_cool,0)<CONFIG["LOSS_COOLDOWN_SEC"]: return
    notional_cap=notional_per_mode(mode)
    if notional_cap<=0: return
    qty, price=size_from_notional(ex, markets, symbol, notional_cap)
    if qty<=0: return
    tp=price*(1+CONFIG["TP_PCT"]/100.0); sl=price*(1-CONFIG["SL_PCT"]/100.0)
    _, tp=clamp_qty_price(ex, symbol, qty, tp); _, sl=clamp_qty_price(ex, symbol, qty, sl)
    notional_entry=price*qty; entry_fee=compute_fee(notional_entry)
    if CONFIG["LIVE"]:
        try: ex.create_order(symbol,"market","buy",qty)
        except Exception as e: print(f"[{ts()}] [OPEN/ERR] {symbol} {mode} {e}"); return
    positions[symbol]={"mode":mode,"entry":price,"qty":qty,"tp":tp,"sl":sl,"opened_at":now_ts(),"entry_fee":entry_fee}
    try:
        if notifier and notifier.enabled(): notifier.send_trade_open(symbol, mode, qty, price, tp, sl, context)
        else: tg(f"[{ts()}] [OPEN] {symbol} {mode.upper()} qty={qty:.8f} @ {price:.4f} tp={tp:.4f} sl={sl:.4f} {context}")
    except Exception: pass

def maybe_close(ex, symbol: str):
    global equity, STATS
    pos=positions.get(symbol); if not pos: return
    price=fetch_ticker_price(ex, symbol)
    should_close=False; reason=None
    if price>=pos["tp"]: should_close=True; reason="TP"
    elif price<=pos["sl"]: should_close=True; reason="SL"
    elif now_ts()-pos.get("opened_at", now_ts())>=CONFIG["TIMEOUT_MIN"]*60: should_close=True; reason="TIMEOUT"
    if not should_close: return
    notional_entry=pos["entry"]*pos["qty"]; notional_exit=price*pos["qty"]
    entry_fee=pos.get("entry_fee", compute_fee(notional_entry)); exit_fee=compute_fee(notional_exit)
    gross_pnl=(price-pos["entry"])*pos["qty"]; pnl_net=gross_pnl-(entry_fee+exit_fee)
    if CONFIG["LIVE"]:
        try: ex.create_order(symbol,"market","sell",pos["qty"])
        except Exception as e: print(f"[{ts()}] [CLOSE/ERR] {symbol} {pos['mode']} sell failed: {e}")
    equity+=pnl_net
    try: stats_on_close(STATS, gross_pnl, (entry_fee+exit_fee), pnl_net, notional_exit); save_stats(STATS)
    except Exception: pass
    try:
        if notifier and notifier.enabled(): notifier.send_trade_close(symbol, pos['mode'], reason, gross_pnl, (entry_fee+exit_fee), pnl_net)
        else: tg(f"[{ts()}] [CLOSE] {symbol} {pos['mode'].upper()} reason={reason} gross={gross_pnl:+.6f} fees={(entry_fee+exit_fee):.6f} pnl={pnl_net:+.6f}")
    except Exception: pass
    if pnl_net<0: last_loss_time[(symbol,pos["mode"])]=now_ts()
...
def main():
    ...
    try:
        ...
        STATS=load_stats(); ex=build_exchange(); markets=load_markets_retry(ex)
        boot=boot_summary(); print(boot)
        try:
            ...
            print(totals_line); 
            if notifier and notifier.enabled(): notifier.send(totals_line)
        except Exception: pass
        try:
            if notifier and notifier.enabled(): notifier.send(boot)
        except Exception as _e:
        ...
        while RUNNING:
            ...
            for symbol in CONFIG["SYMBOLS"]:
                try:
                    ...
                    if now_ts()-last_sig<CONFIG["SIGNAL_COOLDOWN"]:
                        maybe_close(ex, symbol); continue
                    ...
                    try:
                        last_price=closes[-1]; debug_throttled(symbol, f"Price: {last_price:.2f} | RSI: {rsi_val:.2f}", every_sec=5.0)
                    except Exception: pass
            ...
            try: maybe_send_summary(STATS)
            except Exception: pass
            elapsed=now_ts()-loop_start; time.sleep(max(1.0 - elapsed, 0.1))
    finally:
        print(f"[{ts()}] [EXIT] Loop stopped."); release_lock()
